...
class AudioExporter:
    ...
    def save_to_wav(self, audio: np.ndarray, filename: str, volume: float = 1.0) -> Optional[str]:
        """
        ...
        try:
            ...
            sf = get_soundfile_module() if HAS_SOUNDFILE else None
            ...
            logger.error(f"Error saving WAV file to {filename}: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error saving WAV file: {e}")
            return None

    def save_to_mp3(self, audio: np.ndarray, filename: str, bitrate: str = "320k") -> Optional[str]:
        """
        ...
        try:
            ...
            wav_result = self.save_to_wav(audio, temp_wav)
            
            # Make sure temporary WAV was created successfully
            if wav_result is None:
                logger.error("Failed to create temporary WAV file for MP3 conversion")
                return None
            
            # Get AudioSegment if available
            AudioSegment = get_pydub_audiosegment()
            ...
            if not HAS_PYDUB or AudioSegment is None:
                ...
                try:
                    # Ensure directory exists
                    os.makedirs(os.path.dirname(wav_filename) if os.path.dirname(wav_filename) else ".", exist_ok=True)
                    os.rename(temp_wav, wav_filename)
                    return wav_filename
                except OSError as e:
                    logger.error(f"Error saving fallback WAV file: {e}")
                    return None
